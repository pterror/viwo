...
createLibraryTester(Time, "Time Library", (test) => {
  ...
  test("time.format", () => {
    ...
    expect(typeof evaluate(Time.timeFormat(dateStr, "time"), ctx)).toBe(
      "string",
    );
    expect(typeof evaluate(Time.timeFormat(dateStr, "date"), ctx)).toBe(
      "string",
    );
    expect(typeof evaluate(Time.timeFormat(dateStr, "full"), ctx)).toBe(
      "string",
    );
  });
  ...
  test("time.from_timestamp", () => {
    ...
    expect(evaluate(Time.timeFromTimestamp(ts), ctx)).toBe(
